...
public class KnowledgeGraphBuilder {
    ...
    private void buildExtendRelation(Stream<ClassOrInterfaceDeclaration> classDeclarations) {
        classDeclarations.forEach(declaration -> {
            var qualifiedName = declaration.resolve().getQualifiedName();
            var typeEntity = entityManager.getTypeEntityOrCreate(qualifiedName);
            var parentTypes = Stream.concat(declaration.getExtendedTypes().stream(), declaration.getImplementedTypes().stream());
            ...
            typeEntity.setIterableType(getIterableType(declaration.resolve()));
        });
    }
    ...
    private void buildMethodExtendRelation(Stream<CompilationUnit> cus) {
        cus.map(cu -> cu.findAll(MethodDeclaration.class))
                ...
                .forEach(declaration -> {
                    try {
                        var resolvedMethod = (JavaParserMethodDeclaration) declaration.resolve();
    ...
    private void buildProduceRelation(List<CompilationUnit> cus) {
        cus.stream().map(cu -> cu.findAll(MethodDeclaration.class))
                ...
                .forEach(declaration -> {
                    try {
                        var resolvedMethod = declaration.resolve();
    ...
    private Optional<TypeEntity> type2entity(ClassOrInterfaceType declaration) {
        try {
            var qualifiedName = declaration.resolve().getQualifiedName();
    ...
    public void build() {
        ...
        System.out.println("Number of class or interface: " + classOrInterfaceDeclarations.size());
